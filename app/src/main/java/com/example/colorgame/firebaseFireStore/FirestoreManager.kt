...
class FirestoreManager(private val db: FirebaseFirestore) {
    ...
    fun addUserIfDoesNotExist(playerName: String, initScore: Map<String, Int>, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        db.collection("users").document(playerName).get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                val document = task.result
                if (document != null && document.exists()) {
                    Log.i("TAG", "Document with playerName $playerName already exists")
                } else {
    ...
    fun initPlayerScore(playerName: String, initScore: Map<String, Int>, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        val documentReference = db.collection("users").document(playerName)

        documentReference.update(initScore)
            .addOnSuccessListener {
    ...
    fun removeAllScoreListeners() {
        for (listener in scoreListeners.values) {
            listener.remove()
        }
